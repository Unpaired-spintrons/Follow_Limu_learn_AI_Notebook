...
对于一个模型，我们如果在一个很大的模型下时，我们会发现一个问题就是我们的顶部梯度较大，而底部梯度很小。

> 原理解释：我们的梯度计算其实就是从顶部到底部，由上到下的一次次乘积。我们发现我们计算出一个梯度值之后(这个值一般会比较小)，我们再去一层层计算下一个梯度值，最终我们乘了很多层之后我们就会出现$0.1 \times 0.1 \times 0.1……=10^{-n}$这种指数缩小的效应。于是就出现我们上面变化得很好，但是下面变化太慢，导致下面有真正变化之后我们上面的数据反而又要再训练，导致上下变化速率及其不匹配，最终收敛变慢了。

这里李沐老师总结如下：

![1706175738154](image/跟着李沐学AI/1706175738154.png)

### 批量归一化核心想法

最简单的一个思想的版本，其实就是由于我们网络极大时，均值和方差在各层分布差异极大。为了解决这个问题，因此我们提出了一种方法去优化数据输出的均值和方差。也就是说我们乘上一个系数！变换出一个新的均值和方差，然后我们依据这个新的均值和方差进行计算，由于各层分布差异数值上不会差异太大，就可以使得我们的反向传播变化为$1 \times 1 \times ……\approx 1$这样我们的变化就可以被较为稳定的传递到了。

具体做法式子如下：

![1706176410741](image/跟着李沐学AI/1706176410741.png)

![1706176454390](image/跟着李沐学AI/1706176454390.png)

### 批量归一化在做什么

 因为效果实在太好，人们研究发现，其实批量归一化就是往每个小批量里加噪音来控制模型复杂度。

![1706177556053](image/跟着李沐学AI/1706177556053.png)

> 其实从我的理解上而言并不能够将这件事表明是一个噪声问题，它应该被归类为是一个集合减少的过程，其实这里可以说是我们将这个信息的差异值保留。而不是去全局记忆某一个，数据的全部。这其实我可以去类似到我最近的一些工作，比如说：viterbi decoder。我在编写viterbi decoder代码的时候我们会遇到一个问题，或者说我们在编写硬件代码的时候都会遇到一个问题，就是我们如何使用更少的面积来取得不错的效果。这就主要涉及到一个域使用的问题。

这个就是涉及一个域的使用问题。

利用偏移使得我们的学习率变化速度大致一致<当然，这个东西其实有可能是存疑的>

不会改变模型精度只是去增加了收敛

### 批量归一化代码实现

[代码实现](带代码的实操笔记极其函数库解\11现代优化操作方法\batch_normalization.ipynb)
...
![1706444182452](image/跟着李沐学AI/1706444182452.png)

统计机器学习中其实是有一个统计偏差的，不同的模型其实可能会无形中改变自己的作用域，最终稿导致我们的模型可能与正确值渐行渐远！【如左图】，而ResNet提出的观点是在模型优化中进行包含！【新的优化的模型包含老的模型区域，最终实现越来越好<右图>】

#### 残差块

![1706444667823](image/跟着李沐学AI/1706444667823.png)

原本我们寄希望于串联，但现在我们做了一个有趣的决定，我们去**加**这个函数。者就可以做到即使我图中的$f(x)$不抵用，我仍然可用$x$直接的结果。

##### 具体操作

![1706444929338](image/跟着李沐学AI/1706444929338.png)

左边那个是适用于不变通道数的【其中的虚线就是加入的增加的模型】，右边那个适用于变化的通道数，其中$1 \times 1 \,\, Conv$是用于变换通道数的。

##### 不同的残差块

![1706445197323](image/跟着李沐学AI/1706445197323.png)

##### ResNet架构

![1706445380805](image/跟着李沐学AI/1706445380805.png)

##### ResNet与ResConnection

ResNet是一个CV的图形架构，而ResConnection【Residual Connection】是其中最为重要的一种连接方式，也就是直接将之前的输入加进来这种方法受到了广泛的使用！

### ResNet代码实现

[代码实现](带代码的实操笔记极其函数库解\12卷积的经典ResNet\ResNet.ipynb)
